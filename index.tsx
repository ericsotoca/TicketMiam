...
const App = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('tm_history_v3');
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: {
          parts: [
            ...
            { text: "Analyse ce ticket Intermarché. Décode les produits (ex: BOITE X20 NEIGE = 20 Oeufs Score A, PSDT = Pâturages). Retourne le nom du magasin et la liste des produits alimentaires avec Nutri-Score (A-E) et macros pour 100g." }
          ]
        },
      ...
      const avg = result.products.reduce((acc: any, p: any) => acc + (scores[p.nutriScore as keyof typeof scores] ?? 2), 0) / result.products.length;
      ...
      const scanResult: ScanResult = {
        ...
        products: result.products.map((p: any) => ({ ...p, id: Math.random().toString(36) })),
        totalScore: global
      };
      ...
      localStorage.setItem('tm_history_v3', JSON.stringify(newHistory));
      setView('result');
    } catch (err) {
      alert("Erreur d'analyse. Assurez-vous d'avoir une bonne connexion et une photo nette.");
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  const chartData = useMemo(() => {
    if (!current) return [];
    const p = current.products.reduce((acc, curr) => acc + curr.proteins, 0);
    const c = current.products.reduce((acc, curr) => acc + curr.carbs, 0);
    const f = current.products.reduce((acc, curr) => acc + curr.fats, 0);
    return [
  ...
  if (isLoading) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-10 bg-slate-900 text-white animate-fadeIn">
      ...
      <h2 className="text-2xl font-black italic">Analyse Intermarché...</h2>
      <p className="text-slate-400 mt-2 text-center">Gemini décrypte vos articles et calcule les scores nutritionnels.</p>
    </div>
  );

  return (
    <div className="max-w-md mx-auto min-h-screen p-6 pb-32 animate-fadeIn">
      {view === 'home' ? (
        <div className="space-y-12 py-10">
          ...
          <label className="flex flex-col items-center justify-center w-full h-80 border-4 border-emerald-500 border-dashed rounded-[3.5rem] bg-white shadow-2xl cursor-pointer hover:bg-emerald-50 transition-all active:scale-95">
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white rounded-[3rem] p-10 shadow-2xl text-center space-y-4 relative overflow-hidden border border-slate-100">
            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Bilan Nutritionnel</p>
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
             <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Répartition Moyenne</h3>
             <NutritionChart data={chartData} />
          </div>

          <div className="space-y-3">
            <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest px-4">Articles identifiés</h3>
      ...
      <nav className="fixed bottom-8 left-8 right-8 bg-white/90 backdrop-blur-md px-10 py-5 flex justify-around items-center rounded-full shadow-2xl z-50 border border-slate-100">
        ...
        <button onClick={() => { if(confirm("Vider l'historique ?")) { localStorage.clear(); setHistory([]); setView('home'); }}} className="text-2xl text-slate-300"><i className="fas fa-trash"></i></button>
      </nav>
    </div>
  );
};
